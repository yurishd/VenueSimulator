...
class BaseFixVenueClientStateStreet(MFFixVenueClient):
    def CreateInitialMDEntry(self, ccyN, bidPrices, ofrPrices):
        ...
        for i in range(len(bidPrices)):
            rv = rv + [
                        ...
                        ('278', '_b{n}'.format(n=i)), # MDEntryID
                        ('55',  ccy),
                        ('65',  'SP'), # SymbolSfx
                        ('270', bidPrices[i][0]),  # Price
                        ('15', ccy[:3]), # Currency
                        ('271', bidPrices[i][1]),  # Size
                        ('282',  'MDEntryOriginatorBid_{n}'.format(n=i)), # MDEntryOriginator
                 ]
        for i in range(len(ofrPrices)):
            rv = rv + [
                        ('279', '0'), # MDUpdateAction NEW(0)
                        ('269', '1'), # MDEntryType OFFER(1)
                        ('278', '_o{n}'.format(n=i)), # MDEntryID
                        ('55',  ccy),
                        ('65',  'SP'), # SymbolSfx
                        ('270', ofrPrices[i][0]),  # Price
                        ('15', ccy[:3]), # Currency
                        ('271', ofrPrices[i][1]),  # Size
                        ('282',  'MDEntryOriginatorOfr_{n}'.format(n=i)), # MDEntryOriginator
                 ]
        return rv
    ...
    def SendMDRequestReject((lport, rport), req, rjReason, text):
    ...
    def OnMarketDataRequest(self, (lport, rport), data):
        ...
        if self.IsCcySupported(req.ccy):
            ...
            self.SendMDRequestReject((lport, rport), req, '0', 'Unsupported symbol')
...
def main():
    ...
    venue = MFFixVenue()
    cl = BaseFixVenueClientStateStreet(args)
