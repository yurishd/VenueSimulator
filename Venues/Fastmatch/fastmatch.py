...
class BaseFixVenueClientFastmatch(MFFixVenueClient):
    def CreateInitialMDEntry(self, ccyN, bidPrices, ofrPrices, trades):
        ccy = self.ccys[ccyN]
        rv = [
            ('262', None), # MDReqID will be provided when request arrived
            ('268', str(len(bidPrices) + len(ofrPrices) + len(trades)))
            ]
        for i in range(len(bidPrices)):
            rv = rv + [
                        ...
                        ('278', 'YSHBidEntryID_{ccy}{n}'.format(ccy=ccy,n=i)),  # MDEntryID
        ...
        for i in range(len(ofrPrices)):
            rv = rv + [
                        ('279', '0'), # MDUpdateAction, NEW(0)
                        ('55',  ccy),
                        ('269', '1'), # MDEntryType OFFER(0)
                        ('278', 'YSHOfrEntryID_{ccy}{n}'.format(ccy=ccy,n=i)),  # MDEntryID
    ...
    def __init__(self, args):
        if not hasattr(self, 'name'): # Derived may set it up
            self.name = "Base Fix Client Fastmatch" # Must be before super
        super(BaseFixVenueClientFastmatch, self).__init__(args)
        ...
        self.initialMDEntries = {
            self.ccys[0] : self.CreateInitialMDEntry( 0, # 'USD/JPY'
                ...
                trades=[('113.077','250000'), ('113.080','500000')]
                    ),
    ...
    def SendTestTradeData(self, (lport, rport), data):
        self.SendFixDataToServerAsVenue( {
                    'lport' : lport,
                    'rport' : rport,
...
def main():
    ...
    venue = MFFixVenue()
    cl = BaseFixVenueClientFastmatch(args)
