...
class BaseFixVenueClientHSBC(MFFixVenueClient):
    def CreateInitialMDEntry(self, ccyN, tag52, bidPrices, ofrPrices):
        dnow = tag52[:8]
        ccy = self.ccys[ccyN]
        rv = [
            ('55',  ccy),
        ...
        for i in range(len(bidPrices)):
            entryId = 'QuoteEntryID{ccy1}{ccy2}Bid{i}'.format(ccy1=ccy[:3], ccy2=ccy[4:], i=i)
            self.myQuotes.add(entryId)
            rv = rv + [
        ...
        for i in range(len(ofrPrices)):
            entryId = 'QuoteEntryID{ccy1}{ccy2}Ofr{i}'.format(ccy1=ccy[:3], ccy2=ccy[4:], i=i)
            self.myQuotes.add(entryId)
            rv = rv + [
    ...
    def __init__(self, args):
        if not hasattr(self, 'name'): # Derived may set it up
            self.name = "Base Fix Client HSBC" # Must be before super
        super(BaseFixVenueClientHSBC, self).__init__(args)
    ...
    def IsQuoteIdValid(self, quoteIdToTest):
        return quoteIdToTest in self.myQuotes
    ...
    def NoticeMDLoggedOn(self, (lport, rport)):
        self.Log("MD connection is established");

    def NoticeOFLoggedOn(self, (lport, rport)):
        self.Log("OF connection is established");
...
def main():
    ...
    venue = MFFixVenue()
    cl = BaseFixVenueClientHSBC(args)
